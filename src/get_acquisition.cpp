...
int get_acquisition_kinetic(const AndorParameters *params) noexcept {
  ...
  if (error != DRV_SUCCESS) {
    if (error == DRV_ERROR_ACK) {
      fprintf(stderr,
              "[ERROR] Failed to get image; Unable to communicate with card\n");
    } else if (error == DRV_P1INVALID) {
      fprintf(stderr, "[ERROR] Failed to get image; Invalid data pointer\n");
    } else if (error = DRV_P2INVALID) {
      fprintf(stderr, "[ERROR] Failed to get image; Incorrect array size\n");
    }
  }
}