...
int get_rta_scan(const AndorParameters *params, FitsHeaders *fheaders,
  ...
  if (unsigned error = StartAcquisition(); error != DRV_SUCCESS) {
    ...
    fprintf(stderr, "[ERROR][%s] Failed to start acquisition; error descrition %s:\n",
            date_str(buf), get_start_acquisition_status_string(error, acq_str));
    
    socket_sprintf(socket, sockbuf,
                   "done;error:%u;info:start acquisition error, image %d/%d;time:%s;", error, 1,
                   1, date_str(buf));
  ...
  // acquisition to end
  std::thread report_t(
    rta_lambda, AcquisitionSeriesReporter(&socket, (long)(exposure*1000),
                              params->num_images_, std::chrono::high_resolution_clock::now()));
    ...
    if (status != DRV_SUCCESS) {
      ...
      if (abort_set) {
        fprintf(stderr,
                ...
                date_str(buf), __func__);
        socket_sprintf(socket, sockbuf,
                       "done;status:unfinished %d/%d (abort called by user);error:%d;time:%s;",
                       cur_img_in_series, params->num_images_, status, date_str(buf));
      } else {
        socket_sprintf(socket, sockbuf,
                       "done;status:failed/error %d/%d while waiting acquisition;error:%d;time:%s;",
                       cur_img_in_series, params->num_images_, status, date_str(buf));
      }
      return 1;
    } // WaitForAcquisition()
...
#ifdef DEBUG
  printf(">> WaitForAcquisition took %ld millisec (image %d/%d)\n", std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - wfa_ci).count(), cur_img_in_series, params->num_images_);
#endif
  ...
  auto gi_ci = std::chrono::system_clock::now();
...
#ifdef DEBUG
      if (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - tt).count() > 10) {
...
#ifdef DEBUG
    printf(">> GetNumberNewImages(&vfirst, &vlast) returned vfirst=%d and vlast=%d\n", vfirst, vlast);
#endif

    // get current image from circular buffer; copy to img_buffer
    error = GetImages(cur_img_in_series, cur_img_in_series, img_buffer, xpixels*ypixels, &vfirst, &vlast);

#ifdef DEBUG
    printf(">> GetImages for image %d returned sizes: validfirst:%d, validlast:%d\n", cur_img_in_series, vfirst, vlast);
#endif
    ...
    if (error != DRV_SUCCESS) {
      ...
      fprintf(stderr,
              "[ERROR][%s] Failed retrieving acquisition from cammera buffer! Error: %s"
              "(traceback: %s)\n",
      ...
      socket_sprintf(socket, sockbuf,
        "done;status:failed/error image %d/%d while retrieving data (%s);error:%u;time:%s;",
                       cur_img_in_series, params->num_images_, errorbuf, error, date_str(buf));
      return 1;
    }

#ifdef DEBUG
  printf(">> GetImage took %ld millisec (image %d/%d)\n", std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - gi_ci).count(), cur_img_in_series, params->num_images_);
  ...
  printf("[DEBUG][%s] Image acquired and saved to buffer for %d/%d\n", date_str(buf), cur_img_in_series, params->num_images_);
    ...
    if (get_next_fits_filename(params, fits_filename)) {
      ...
      report_t.join();
      abort_t.join();
      socket_sprintf(socket, sockbuf, "done;status:error saving FITS file (%d/%d);error:%d;time:%s;", cur_img_in_series, params->num_images_, 1, date_str(buf));
      return 1;
    }
    ...
    if (fits.write<at_32>(img_buffer)) {
      ...
      report_t.join();
      abort_t.join();
      socket_sprintf(socket, sockbuf, "done;error:1;status:error while saving to FITS, image (%d/%d);error:%d;time:%s;", cur_img_in_series, params->num_images_, 2, date_str(buf));
      return 2;
    } else {
...
#ifdef DEBUG
  printf(">> SaveToFits took %ld millisec (image %d/%d)\n", std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - saf_ci).count(), cur_img_in_series, params->num_images_);
#endif

  } // colected/saved all exposures!

#ifdef DEBUG
  printf(">> Series took %ld millisec\n", std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - series_start).count());
#endif
  ...
  socket_sprintf(socket, sockbuf,
                 "done;error:0;info:exposure series ok;status:acquired and saved %d/%d images;time:%s;", cur_img_in_series, params->num_images_, date_str(buf));
