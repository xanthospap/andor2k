...
int cool_to_temperature(int tempC, const andor2k::Socket *socket) noexcept {
  ...
  printf("[DEBUG][%s] Current camera temperature is %+3dC\n", date_str(buf),
         current_temp);
  if (socket)
    socket_sprintf(*socket, sbuf, "ctemp:%d;status:got current camera temperature", current_temp);
  ...
  if (status != DRV_SUCCESS) { // setting temperature failed!
    fprintf(stderr, "[ERROR][%s] Failed to set target temperature! (traceback: %s)\n", buf, __func__);
    if (socket)
      socket_sprintf(*socket, sbuf, "done;error:%u;ctemp:%d;status:Failed to set target temperature!", status, current_temp);
    //switch (status) {
  ...
  if (status != DRV_SUCCESS) { // failed to start cooler
    fprintf(stderr, "[ERROR][%s] Failed to startup the cooler! (traceback: %s)", buf, __func__);
    if (socket)
      socket_sprintf(*socket, sbuf, "done;error:%u;ctemp:%d;status:Failed to startup the cooler!", status, current_temp);
    //switch (status) {
    //case DRV_NOT_INITIALIZED:
  ...
  while (status != DRV_TEMP_STABILIZED) {
    ...
    if (status == DRV_TEMP_NOT_REACHED || status == DRV_TEMP_DRIFT || status == DRV_TEMP_NOT_STABILIZED || DRV_TEMP_OFF) {
      printf("[DEBUG][%s] Temperature: %+4dC; %s\n", date_str(buf), current_temp, status_str);
      if (socket)
        socket_sprintf(*socket, sbuf, "ctemp:%d;status:%s (%u)", current_temp, status_str, status);
    } else {
      fprintf(stderr, "[ERROR][%s] Cooling failed! %s (traceback: %s)\n", date_str(buf), status_str, __func__);
      if (socket)
        socket_sprintf(*socket, sbuf, "done;error:%u;ctemp:%d;status:%s", status, current_temp, status_str);
      return 1;
    }
    ...
    if (elt > MAX_COOLING_DURATION) {
      fprintf(stderr,
              ...
              __func__);
      if (socket)
        socket_sprintf(*socket, sbuf, "done;error:%u;ctemp:%d;status:Failed to reach temperature after %3ld minutes", 1, current_temp, elt);
      return 1;
    } else {
  ...
  if (socket)
      socket_sprintf(*socket, sbuf, "done;ctemp:%d;status:Target temperature reached and stabilized", current_temp);
  return 0;
}
