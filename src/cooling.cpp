...
int cool_to_temperature(int tempC) noexcept {
  ...
  (int)params->cooler_mode_); status = SetCoolerMode(params->cooler_mode_); if
  (status != DRV_SUCCESS) { fprintf(stderr, "[ERROR][%s] Failed to set cooler
  mode!", tbuf); switch (status) { case DRV_NOT_INITIALIZED: fprintf(stderr, "
  System is not initialized\n"); return 1; case DRV_ACQUIRING: fprintf(stderr, "
  Acquisition in progress\n"); return 2; case DRV_P1INVALID: fprintf(stderr, "
  Temperature invalid\n"); return 4; case DRV_NOT_SUPPORTED: fprintf(stderr, "
  The camera does not support this mode\n"); return 5; default: fprintf(stderr,
