...
int coarse_exposure_time(const AndorParameters *params,
  ...
  if (unsigned error = GetAcquisitionTimings(&exposure, &accumulate, &kinetic);
      error != DRV_SUCCESS) {
    return 1;
  }
  ...
  case AcquisitionMode::SingleScan:
    ...
    printf("\texposure ms: %ld\n", exposure_ms);
    printf("\treadout ms : %ld\n", readouttime_ms);
    printf("\tperimage ms: %ld\n", millisec_per_image);
    break;

  case AcquisitionMode::Accumulate:
    total_millisec =
        (params->num_accumulations_ - 1) * (accumulate_ms - exposure_ms) +
        exposure_ms + readouttime_ms;
  ...
  case AcquisitionMode::KineticSeries:
    millisec_per_image =
        ...
        exposure_ms + readouttime_ms;
    total_millisec =
        (params->num_images_ - 1) * kinetic_ms + millisec_per_image;
    break;
