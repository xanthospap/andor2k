...
int sig_interrupt_set = 0;
  ...
  char *get_status_string(char *buffer) noexcept {
    ...
    case DRV_ERROR_ACK:
      ...
      break;
    case DRV_ACQ_BUFFER:
      std::strcpy(buffer, "Computer unable to read the data via the ISA slot at the required rate");
      break;
    case DRV_ACQ_DOWNFIFO_FULL:
      std::strcpy(buffer, "Computer unable to read data fast enough to stop camera memory going full");
      break;
    case DRV_SPOOLERROR:
