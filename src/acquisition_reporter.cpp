...
void AcquisitionReporter::report() noexcept {
  ...
  while (!lk.try_lock()) {
    ...
    from_thread_start = std::chrono::duration_cast<std::chrono::milliseconds>(
                            t_now - series_start)
                            .count();
  ...
  date_str(mbuf + len_const_prt);
  std::sprintf(mbuf + std::strlen(mbuf),
               ";progperc:%d;sprogperc:%d;elapsedt:%.2f;selapsedt:%.2f", 100,
               series_done, from_thread_start / 1e3, from_thread_start / 1e3);
