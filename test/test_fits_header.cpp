...
int main() {
    ...
    if (headers.update<double>(" Key94 ", 10.2, "a double value (changed!)")<0) {
        fprintf(stderr, "Failed adding header at line %d\n", __LINE__);
    }
    ...
    if (headers.update<int>("Key92", 10, "an signed(!) integer value; should be an error")<0) {
        fprintf(stderr, "Failed adding header at line %d\n", __LINE__);
    }
    // key93 has a float value
    if (headers.update<double>("Key93", 10e0, "a double(!) value; should be an error")<0) {
        fprintf(stderr, "Failed adding header at line %d\n", __LINE__);
    }
