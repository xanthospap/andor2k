...
template <>
char *strfdt<DateTimeFormart::YMDHMfS>(const time_point &t,
                                       char *buf) noexcept {
  long fsec;
  auto tm = strfdt_work(t, fsec);
  if (int btw = std::strftime(buf, 64, "%FT%T.", &tm); btw > 0) {
...
void Exposure_TimeSpec_To_UtStart_String(struct timespec time,
                                         char *time_string);
void Exposure_TimeSpec_To_Date_Obs_String(struct timespec time,
                                          char *time_string);
...
int main() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    auto TimeSinceLastImage =
        (mr_current_time.tv_sec + mr_current_time.tv_nsec / 1e9) -
        (LastImageTime.tv_sec + LastImageTime.tv_nsec / 1e9);
    /*
    auto TimeSinceLastImage = static_cast<double>(mr_current_time.tv_sec -
    LastImageTime.tv_sec); double nsec = (mr_current_time.tv_nsec -
    LastImageTime.tv_nsec)/1e9; if (nsec>=0) { TimeSinceLastImage += nsec; }
    else { TimeSinceLastImage -= 1e0; TimeSinceLastImage += (1e9 - nsec);
    }*/
    ...
    std::memset(exposure_utstart, 0, 64);
    printf("\t --> dif2 = %s - %s\n",
           strfdt<DateTimeFormart::HMfS>(mr_current_time__, buffer),
           strfdt<DateTimeFormart::HMfS>(LastImageTime__, exposure_utstart));
    ...
    Exposure_TimeSpec_To_Date_String(Exposure_Start_Time, exposure_date);
    Exposure_TimeSpec_To_UtStart_String(Exposure_Start_Time, exposure_utstart);
...
void Exposure_TimeSpec_To_Date_Obs_String(struct timespec time,
  ...
  milliseconds =
      (long)(((double)time.tv_nsec) / ((double)CCD_GLOBAL_ONE_MILLISECOND_NS));
...
/**
 * Routine to convert a timespec structure to a DATE sytle string to put into a
 * FITS header. This uses gmtime and strftime to format the string. The
 * resultant string is of the form: <b>CCYY-MM-DD</b>, which is equivalent to
 * %Y-%m-%d passed to strftime.
 * @param time The time to convert.
 * @param time_string The string to put the time representation in. The string
...
int correct_start_time(struct timespec *t) {
  /* This function applies the time correction derived in
...
int correct_start_time(
    std::chrono::time_point<std::chrono::high_resolution_clock> &t) noexcept {
  t -= TimeCorrection__;
  return 0;
}
