...
int resolve_image_parameters(const char* command = buffer, AndorParameters& aparams = params) noexcept {
    ...
    while (token) {
        ...
        * --------------------------------------------------------*/
        ...
        std::memcpy(aparams.image_filename_, token,
                    std::strlen(token));
...
int setup_image(int &xpixels, int& ypixels, const AndorParameters& aparams = params) noexcept {
  // set read mode
  ...
  printf("[DEBUG][%s] Width  = %5d - %5d = %5d\n", date_str(), aparams.image_hend_,
         aparams.image_hstart_, width);
  printf("[DEBUG][%s] Height = %5d - %5d = %5d\n", date_str(), params.image_vend_,
         aparams.image_vstart_, height);
  ...
  printf("[DEBUG][%s] Shutter initialized to closing/opening delay: %3d/%3d "
         "milliseconds\n", date_str(),
         aparams.shutter_closing_time_, aparams.shutter_opening_time_);
  printf("[DEBUG][%s] Shutter mode : %1d\n", date_str(), ShutterMode2int(aparams.shutter_mode_));
...
int acquire_image(AndorParameters& aparams, int xpixels, int ypixels) noexcept {
  ...
  if (aparams.acquisition_mode_ == AcquisitionMode::SingleScan) {
    ...
    printf("[DEBUG][%s] Image acquired; saving to FITS file \"%s\" ...", date_str(), fits_file);
    ...
    auto sleep_ms = std::chrono::milliseconds(
        static_cast<int>(std::ceil(aparams.exposure_ + 0.5)) * 1000);
    ...
    while ((images_acquired < aparams.num_images_) && !abort) {
      ...
      case DRV_SUCCESS:
        ...
        printf("[DEBUG][%s] Image acquired; saving to FITS file \"%s\" ...", date_str(), 
               fits_file);
        if (SaveAsFITS(fits_file, 3) != DRV_SUCCESS) {
          fprintf(stderr, "\n[ERROR][%s] Failed to save image to fits format!\n", date_str());
        } else {
      ...
      case DRV_NOT_INITIALIZED:
        ...
        [[fallthrough]];
      case DRV_ERROR_ACK:
        fprintf(stderr, "[ERROR][%s] Failed to acquire image; unable to "
                        "communicate with card\n", date_str());
        [[fallthrough]];
      case DRV_P1INVALID:
        fprintf(
            stderr,
            "[ERROR][%s] Failed to acquire image; data array size is incorrect\n", date_str());
        AbortAcquisition();
        abort = true;
        break;
